...
每条回答显示等待、生成、完成或失败状态；只有正常结束且收到非空回答才显示完成，达到长度上限会单独提示。流式文本经过浏览器端队列平滑为打字机效果，系统启用“减少动态效果”时立即显示。等待提示表示请求尚未完成，不展示模型内部推理内容。

Qwen3.8-27B 在 AMD 共享端点上默认会思考，且只接受 `low`、`medium`、`xhigh`。页面提供这三档选择并默认 `low`；服务端按白名单转为 `reasoning_effort`，不会把内部 `reasoning` 文本转发给浏览器。该模型不接受 `none`，因此不能提供真正的“完全关闭思考”。
