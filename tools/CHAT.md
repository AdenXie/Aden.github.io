...
`tools/build-site.cjs` 在生成的 `vercel.json` 顶层设置 `regions: ['hnd1']`（日本东京），所有服务端接口（包括聊天）使用该地区，聊天不再单独覆盖地区。代码配置优先于 Vercel 项目面板的默认地区；文章、图片等静态文件仍由全球 CDN 分发。请在这里修改配置，勿直接修改生成分支的 `vercel.json`。
