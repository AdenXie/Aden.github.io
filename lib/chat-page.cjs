...
<section id="aden-chat" translate="no" aria-label="${t('临时 AI 对话', 'Temporary AI chat')}">
  <header class="chat-heading"><div><span class="chat-label">ADEN'S SPACE / AI CHAT</span><h2>${t('想聊点什么？', 'What’s on your mind?')}</h2><p>${t('一个问题，一点灵感，或一起理清思路。', 'A question, a spark of inspiration, or room to think.')}</p></div><button type="button" data-chat="clear">${t('清空对话', 'Clear chat')}</button></header>
