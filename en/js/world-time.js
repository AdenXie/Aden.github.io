(() => {
  ...
  function formatter(zone, kind) {
    ...
    if (!formatters.has(key)) {
      ...
      formatters.set(key, new Intl.DateTimeFormat(kind === 'date' ? "en-GB" : 'en-GB', { ...options, timeZone: zone }));
    }
  ...
  function compare(local, remote) {
    ...
    const duration = [Math.floor(minutes / 60) ? `${Math.floor(minutes / 60)} hours` : '', minutes % 60 ? `${minutes % 60} minutes` : ''].filter(Boolean).join(' ');
    const days = remote.day - local.day;
    return {
      difference: delta === 0 ? "Same as your local time" : ` ${delta > 0 ? "ahead of you by" : "behind you by"} ${duration}`,
      dayDifference: days === 0 ? "On the same day as you" : `Local date is ${days > 0 ? "later by" : "earlier by"} ${Math.abs(days)} days`
    };
  }
  ...
  function mount() {
    ...
    function render() {
      ...
      try {
        ...
        set('remote-time', '--:--:--');
        set('remote-date', "This browser does not currently support this time zone.");
        set('remote-zone', selected.zone);
        set('difference', "Please update your browser and try again.");
        set('day-difference', '');
      }
    }
    function select(id) {
